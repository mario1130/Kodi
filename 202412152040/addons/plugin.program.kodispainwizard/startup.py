...
def auto_install_repo():
    if not os.path.exists(os.path.join(CONFIG.ADDONS, CONFIG.REPOID)):
        ...
        if response:
                ...
                if repo_response:
                    ...
                    try:
                        ...
                        logging.log_notify("{1}".format(CONFIG.COLOR1, reponame),
                                           "[COLOR {0}]Addon actualizado[/COLOR]".format(CONFIG.COLOR2),
                    ...
                    logging.log_notify("[COLOR {0}]Error al instalar el repositorio[/COLOR]".format(CONFIG.COLOR1),
                                       "[COLOR {0}]URL incorrecta para el zip[/COLOR]".format(CONFIG.COLOR2))
            ...
            logging.log_notify("[COLOR {0}]Error al instalar el repositorio[/COLOR]".format(CONFIG.COLOR1),
                               "[COLOR {0}]Archivo addon.xml incorrecto[/COLOR]".format(CONFIG.COLOR2))
...
def installed_build_check():
    ...
    if not CONFIG.EXTRACT == '100' and CONFIG.EXTERROR > 0:
        ...
        yes = dialog.yesno(CONFIG.ADDONTITLE,
                           '[COLOR {0}]{2}[/COLOR] [COLOR {1}]no fue instalado correctamente[/COLOR]'.format(CONFIG.COLOR1,
                                                                                                   CONFIG.COLOR2,
                                                                                                   CONFIG.BUILDNAME)
                           +'\n'+('Instalado: [COLOR {0}]{1}[/COLOR] / '
                            'Cantidad de errores: [COLOR {2}]{3}[/COLOR]').format(CONFIG.COLOR1, CONFIG.EXTRACT, CONFIG.COLOR1,
                                                                          CONFIG.EXTERROR)
                           +'\n'+'¿Quiere volver a intentarlo?[/COLOR]', nolabel='[B]NO[/B]',
                           yeslabel='[B]SI[/B]')
        ...
        if not CONFIG.SKIN == defaults and not CONFIG.BUILDNAME == "":
            ...
            if not response:
                ...
                dialog.ok(CONFIG.ADDONTITLE,
                          "[COLOR {0}]Parece que la configuracion del skin no se ha aplicado a la build.".format(CONFIG.COLOR2)
                          +'\n'+"Lamentablemente, no se ha incluido ningun parche a la build."
                          +'\n'+"Debe reinstalar la build y asegurarse de forzar el cierre.[/COLOR]")
            else:
                yes = dialog.yesno(CONFIG.ADDONTITLE,
                                       '{0} no se ha instalado correctamente.'.format(CONFIG.BUILDNAME)
                                       +'\n'+'Parece que la configuracion del skin no se ha aplicado a la build.'
                                       +'\n'+'¿Quiere aplicar el Parche?',
                                       nolabel='[B]NO[/B]', yeslabel='[B]SI[/B]')
                if yes:
