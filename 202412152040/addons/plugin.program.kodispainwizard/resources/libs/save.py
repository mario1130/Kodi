...
def import_save_data():
    ...
    if not os.path.exists(TEMP):
        os.makedirs(TEMP)
    source = dialog.browse(1, '[COLOR {0}]Selecciona la ubicacion de SaveData.zip[/COLOR]'.format(CONFIG.COLOR2),
    ...
    overwrite = dialog.yesno(CONFIG.ADDONTITLE,
                                 "[COLOR {0}]¿Prefiere que sobrescribamos todos los archivos de datos guardados o que le preguntemos por cada archivo que se importe?[/COLOR]".format(CONFIG.COLOR2),
                                 yeslabel="[B][COLOR white]Sobreescribir Todos[/COLOR][/B]",
                                 nolabel="[B][COLOR white]No, preguntar[/COLOR][/B]")
            ...
            if os.path.exists(old):
                if overwrite == 1:
                    ...
                    if not dialog.yesno(CONFIG.ADDONTITLE,
                                            "[COLOR {0}]¿Quiere reemplazar el archivo [COLOR {1}]{2}[/COLOR] actual?".format(CONFIG.COLOR2, CONFIG.COLOR1, item),
    ...
    if os.path.exists(super):
        ...
        if os.path.exists(old):
            cont = dialog.yesno(CONFIG.ADDONTITLE,
                                    "[COLOR {0}]¿Quiere reemplazar la carpeta addon_data de Super Favourites por la nueva?".format(CONFIG.COLOR2, CONFIG.COLOR1),
                                    yeslabel="[B][COLOR white]SI[/COLOR][/B]",
                                    nolabel="[B][COLOR white]NO[/COLOR][/B]")
        else:
            cont = 1
    ...
    if x == 0:
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]Fallo al Importar Datos[/COLOR]".format(CONFIG.COLOR2))
    else:
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]Datos Importados correctamente[/COLOR]".format(CONFIG.COLOR2))
...
def export_save_data():
    ...
    source = dialog.browse(3, '[COLOR {0}]Seleccione donde desea exportar el zip de SaveData[/COLOR]'.format(CONFIG.COLOR2),
    ...
    dialog.ok(CONFIG.ADDONTITLE,
                  "[COLOR {0}]Se ha hecho una copia de seguridad de los datos guardados en:[/COLOR]".format(CONFIG.COLOR2)
