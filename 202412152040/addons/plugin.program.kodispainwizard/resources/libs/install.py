...
def wipe():
    ...
    del_file = 0
    progress_dialog.update(0, "[COLOR {0}]Reuniendo lista de excluidos.[/COLOR]".format(CONFIG.COLOR2))
    ...
    progress_dialog.update(0, "[COLOR {0}]Borrando archivos y carpetas:".format(CONFIG.COLOR2))
    ...
    for root, dirs, files in os.walk(xbmcPath, topdown=True):
        ...
        if progress_dialog.iscanceled():
            ...
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]Instalacion Nueva Cancelada[/COLOR]".format(CONFIG.COLOR2))
            return False
    for root, dirs, files in os.walk(xbmcPath, topdown=True):
        dirs[:] = [d for d in dirs if d not in exclude_dirs]
        for name in dirs:
...
def fresh_start(install=None, over=False):
    ...
    if over:
        ...
        yes_pressed = dialog.yesno(CONFIG.ADDONTITLE,
                                       "[COLOR {0}]¿Desea restablecer su".format(CONFIG.COLOR2)
        ...
        yes_pressed = dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]¿Desea restablecer su".format(CONFIG.COLOR2)
                                       +'\n'+"configuracion de Kodi a los valores predeterminados"
                                       +'\n'+"Antes de instalar [COLOR {0}]{1}[/COLOR]?".format(CONFIG.COLOR1, install),
                                       nolabel='[B][COLOR white]NO[/COLOR][/B]',
                                       yeslabel='[B][COLOR white]SI[/COLOR][/B]')
    else:
        yes_pressed = dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]¿Desea restablecer su".format(CONFIG.COLOR2) +' \n' + "configuracion de Kodi a los valores predeterminados?[/COLOR]", nolabel='[B][COLOR white]NO[/COLOR][/B]', yeslabel='[B][COLOR white]SI[/COLOR][/B]')
    if yes_pressed:
        wipe()
        ...
        if over:
            ...
            dialog.ok(CONFIG.ADDONTITLE, "[COLOR {0}]Ahora pulsa OK para forzar el cierre de Kodi y guardar los cambios[/COLOR]".format(CONFIG.COLOR2))
            from resources.libs import update
...
def install_apk(name, url):
    ...
    if tools.platform() == 'android':
        ...
        if os.path.exists(lib):
            redownload = dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {}]{}[/COLOR] ya existe. ¿Quiere volver a descargarlo?'.format(CONFIG.COLOR1, apk),
                               yeslabel="[B]SI[/B]",
                               nolabel="[B]NO[/B]")
            yes = False
        else:
            yes = dialog.yesno(CONFIG.ADDONTITLE,
                                   "[COLOR {0}]Desea descargar e instalar: ".format(CONFIG.COLOR2)
        ...
        if yes or redownload:
            ...
            if not response:
                logging.log_notify(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]Instalador APK: Url de Apk no valida![/COLOR]'.format(CONFIG.COLOR2))
                return
        ...
        dialog.ok(CONFIG.ADDONTITLE, '[COLOR {}]{}[/COLOR] descargado a [COLOR {}]{}[/COLOR]. Si la instalacion no se inicia por si misma, navegue hasta esa ubicacion para instalar la APK.'.format(CONFIG.COLOR1, apk, CONFIG.COLOR1, path))
