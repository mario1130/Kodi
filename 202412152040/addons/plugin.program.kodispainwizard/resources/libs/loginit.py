    ...
    if who == 'all':
        ...
        if os.path.exists(file):
            ...
            logging.log_notify('[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, LOGINID[who]['name']),
                               '[COLOR {0}]Info de Registro: Eliminado[/COLOR]'.format(CONFIG.COLOR2),
                               2000,
                               LOGINID[who]['icon'])
        ...
        if os.path.exists(settings):
            try:
                ...
                logging.log_notify("[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, name),
                                   '[COLOR {0}]Datos de Addon: Borrados[/COLOR]'.format(CONFIG.COLOR2),
                                   2000,
                                   icon)
            except Exception as e:
        ...
        if os.path.exists(LOGINID[who]['path']):
            ...
            if not u or u == '':
                ...
                if dialog.yesno(CONFIG.ADDONTITLE,
                                    "¿Le gustaría guardar el [COLOR {0}]Login Info[/COLOR] para [COLOR {1}]{2}[/COLOR]?".format(CONFIG.COLOR2, CONFIG.COLOR1, n)
                                    +'\n'+"Addon: [COLOR springgreen][B]{0}[/B][/COLOR]".format(u)
                                    +'\n'+"Guardado:[/COLOR] [COLOR red][B]{0}[/B][/COLOR]".format(su) if not su == '' else 'Guardado:[/COLOR] [COLOR red][B]Nada[/B][/COLOR]',
...
def import_list(who):
    if who == 'all':
        ...
        if os.path.exists(LOGINID[who]['file']):
            ...
            logging.log_notify("[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, name),
                       '[COLOR {0}]Datos de Login: Importados[/COLOR]'.format(CONFIG.COLOR2))
...
def activate_login(who):
    if LOGINID[who]:
        if os.path.exists(LOGINID[who]['path']):
            ...
            dialog.ok(CONFIG.ADDONTITLE, '{0} no esta instalado.'.format(LOGINID[who]['name']))
    else:
        xbmc.executebuiltin('Container.Refresh()')
        return
