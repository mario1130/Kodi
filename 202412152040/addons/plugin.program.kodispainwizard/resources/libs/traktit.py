    ...
    if who == 'all':
        ...
        if os.path.exists(file):
            ...
            logging.log_notify("[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, TRAKTID[who]['name']),
                               '[COLOR {0}]Datos Trakt: Eliminados[/COLOR]'.format(CONFIG.COLOR2),
                               2000,
                               TRAKTID[who]['icon'])
        ...
        if os.path.exists(TRAKTID[who]['path']):
            ...
            if not u or u == '':
                ...
                if dialog.yesno(CONFIG.ADDONTITLE,
                                    "¿Quiere guardar los [COLOR {0}]Datos de Trakt[/COLOR] para [COLOR {1}]{2}[/COLOR]?".format(CONFIG.COLOR2, CONFIG.COLOR1, n)
                                    +'\n'+"Addon: [COLOR springgreen][B]{0}[/B][/COLOR]".format(u)
                                    +'\n'+"Guardado:[/COLOR] [COLOR red][B]{0}[/B][/COLOR]".format(su) if not su == '' else 'Guardado:[/COLOR] [COLOR red][B]Ninguno[/B][/COLOR]',
...
def import_list(who):
    if who == 'all':
        ...
        if os.path.exists(TRAKTID[who]['file']):
            ...
            logging.log_notify("[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, name),
                       '[COLOR {0}]Datos de Trakt: Importados[/COLOR]'.format(CONFIG.COLOR2))
...
def activate_trakt(who):
    if TRAKTID[who]:
        if os.path.exists(TRAKTID[who]['path']):
            ...
            dialog.ok(CONFIG.ADDONTITLE, '{0} no esta instalado.'.format(TRAKTID[who]['name']))
    else:
        xbmc.executebuiltin('Container.Refresh()')
        return
