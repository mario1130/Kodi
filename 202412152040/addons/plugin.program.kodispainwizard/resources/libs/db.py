...
def force_check_updates(auto=False, over=False):
    ...
    if not over:
        logging.log_notify(CONFIG.ADDONTITLE,
                           '[COLOR {0}]Forzar la comprobacion de las actualizaciones[/COLOR]'.format(CONFIG.COLOR2))
    ...
    with tools.busy_dialog():
        ...
        for repo in installed_repos.fetchall():
            ...
            logging.log_notify('[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.ADDONTITLE),
                               "[COLOR {0}]{1} Comprobacion forzada satisfactoriamente.[/COLOR]".format(CONFIG.COLOR2, repo))
...
def purge_db_file(name):
    ...
    logging.log_notify("[COLOR {0}]Purgar DB[/COLOR]".format(CONFIG.COLOR1),
...
def purge_db():
    ...
    choice = dialog.multiselect("[COLOR {0}]Seleccione el archivo de base de datos a purgar[/COLOR]".format(CONFIG.COLOR2), display)
    if choice is None or len(choice) == 0:
        logging.log_notify("[COLOR {0}]Purgar Base de Datos[/COLOR]".format(CONFIG.COLOR1),
    ...
    if os.path.exists(dep):
        ...
        for depends in match:
            if 'xbmc.python' not in depends:
                ...
                if dp is not None:
                    dp.update("",
                              "Comprobando Dependencias [COLOR yellow]{0}[/COLOR] para [COLOR yellow]{1}[/COLOR]".format(depends, name),
                              "")
...
def find_binary_addons(addon='all'):
    ...
    if addon == 'all':
        ...
        dialog.ok(CONFIG.ADDONTITLE, "[COLOR {0}]Se han encontrado [COLOR {1}]{2}[/COLOR] addons especificos de plataforma instalados:[/COLOR]".format(CONFIG.COLOR2, CONFIG.COLOR1, len(addonnames)), "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, addonnames))
