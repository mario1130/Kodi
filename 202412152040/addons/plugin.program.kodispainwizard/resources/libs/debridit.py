    ...
    if who == 'all':
        ...
        if os.path.exists(file):
            ...
            logging.log_notify('[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, DEBRIDID[who]['name']),
                               '[COLOR {0}]Debrid Info: Eliminado[/COLOR]'.format(CONFIG.COLOR2),
                               2000,
                               DEBRIDID[who]['icon'])
        ...
        if os.path.exists(DEBRIDID[who]['path']):
            ...
            if not u or u == '':
                ...
                if dialog.yesno(CONFIG.ADDONTITLE,
                                    "¿Te gustaria guardar la [COLOR {0}]Info de Debrid[/COLOR] para [COLOR {1}]{2}[/COLOR]?".format(CONFIG.COLOR2, CONFIG.COLOR1, n)
                                    +'\n'+"Addon: [COLOR springgreen][B]{0}[/B][/COLOR]".format(u)
                                    +'\n'+"Guardado:[/COLOR] [COLOR red][B]{0}[/B][/COLOR]".format(su) if not su == '' else 'Guardado:[/COLOR] [COLOR red][B]Nada[/B][/COLOR]',
                                    yeslabel="[B][COLOR white]Guardar Debrid[/COLOR][/B]",
                                    nolabel="[B][COLOR white]No, Cancelar[/COLOR][/B]"):
                    debrid_it('update', who)
            else:
                debrid_it('update', who)
...
def import_list(who):
    if who == 'all':
        ...
        if os.path.exists(DEBRIDID[who]['file']):
            ...
            logging.log_notify("[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, name),
                       '[COLOR {0}]Debrid Info: Importada[/COLOR]'.format(CONFIG.COLOR2))
    ...
    if DEBRIDID[who]:
        if os.path.exists(DEBRIDID[who]['path']):
            ...
            dialog.ok(CONFIG.ADDONTITLE,
                          '{0} no esta instalado.'.format(DEBRIDID[who]['name']))
    else:
        xbmc.executebuiltin('Container.Refresh()')
        return
