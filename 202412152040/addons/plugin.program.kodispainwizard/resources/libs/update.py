    ...
    if response:
        ...
        if ver > CONFIG.ADDON_VERSION:
            yes = dialog.yesno(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]Hay una nueva version de {1}!'.format(CONFIG.COLOR2, CONFIG.ADDONTITLE)
                                   +'\n'+'¿Desea descargar [COLOR {0}]v{1}[/COLOR]?[/COLOR]'.format(CONFIG.COLOR1, ver),
...
def toggle_addon_updates():
    ...
    setting = '"general.addonupdates"'
    selected = 0
    options = ['Instalar actualizaciones automaticamente', 'Notificar, pero no instalar las actualizaciones', 'No comprobar nunca las actualizaciones']
    ...
    logging.log_notify(CONFIG.ADDONTITLE, 'Las actualizaciones han cambiado a "{0}"'.format(options[selected]))
